    ...
    function report_by_id($id) {
        try{
            $report = DB::table('report.report_list')
                        ...
                        ->select('report.report_list.id', 'report.report_list.no_ticket', 'report.report_list.created_at', 'report.report_list.done_at', 'report.report_segment.level', 'report.report_segment.note', 'report.status.name as status', 'map.irrigations_segment.center_point_json', 'map.irrigations.name as irrigation', 'map.irrigations.type as canal', 'file.upload_dump.file_url as image')
                        ->get();
